...
func main() {
	...
	nearestDistance := math.MaxUint32

	for coord := range wire1Coords {

		if _, ok := wire2Coords[coord]; ok {
			// found intersection, check distance
			d := calcManhattanDistance(coord, "0,0")
			if d < nearestDistance {
				nearestIntersection = coord
				nearestDistance = d
			}
		}
	...
	fmt.Printf("Nearest intersection at (%v) with distance %v\n", nearestIntersection, nearestDistance)
}
